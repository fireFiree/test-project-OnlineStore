...
module.exports.add = function(req, res){
    ...
    //{"product","amount", "address", "delivery", "payment", "status", "declineReason", "orderDate", "buyerName", "buyerEmail"}
    ...
    var order = { product, address, delivery, payment, status, declineReason, orderDate, buyerEmail, buyerName};
        
    let data = fs.readFileSync("./orders/orders.json", "utf8");
    let orders = JSON.parse(data);
        
    const id = Math.max.apply(Math, orders.map(function(o){ return o.id;}));
    order.id = id + 1;
    orders.push(order);
